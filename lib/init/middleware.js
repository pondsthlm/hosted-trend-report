...
function setupAccessLog(app) {
  var logFormat = ":remote-addr - :remote-user [:date[iso]] \":method :url HTTP/:http-version\" :status :res[content-length] :response-time ms";
  var options = {};
  if (config.requestLogging !== "stdout") {
    var accessLogStream = fs.createWriteStream(config.requestLogging, {flags: "a"});
